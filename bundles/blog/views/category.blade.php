	...
	<div class="span9">
		...
		@if (!empty($data))
			<? if (!empty($data['posts']->results)): ?>
				...
				<? endif; ?>
			<? else: ?>
				<div class="alert alert-block">No posts were found in <?=$data['category']->title;?>.</div>
			<? endif; ?>
		@elseif (isset($data))
			<div class="alert alert-error alert-block">Category not found</div>
		@endif

		@if (empty($data) && !empty($categories))
			<ul>
			<? foreach($categories as $category): ?>
				<li><a href="<?=$action_urls['category'];?>/<?=$category->slug;?>" title="<?=$category->title;?>"><?=$category->title;?></a></li>
			<? endforeach; ?>
			</ul>
		@endif
